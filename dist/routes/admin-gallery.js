...
const isVariantHighPath = (path) => path.includes("-high.webp");
const expandVariantPaths = (path) => {
...
adminGalleryRouter.get("/admin/gallery/projects", requireAdmin, async (_req, res) => {
    ...
    return res.status(200).json({ projects: mapProjectRows(data) });
});
adminGalleryRouter.post("/admin/gallery/projects", requireAdmin, upload.fields([
    ...
    try {
        ...
        const uploadFileVariants = async (file, role, index) => {
        ...
        if (extraUploads.length > 0) {
            const { error: extraInsertError } = await supabaseService.from("gallery_project_images").insert(extraUploads.map((image) => ({
